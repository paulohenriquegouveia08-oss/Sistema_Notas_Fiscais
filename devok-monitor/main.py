...
class DevokMonitor:
    ...
    def build_ui(self):
        ...
        # Title
        title = ttk.Label(main, text="Devok Monitor — SisFin Importer", font=("Segoe UI", 14, "bold"))
        title.pack(pady=(0, 10))

        # Folder selection
        folder_frame = ttk.Frame(main)
        folder_frame.pack(fill=tk.X, pady=2)
        ttk.Label(folder_frame, text="Pasta Devok:", width=12).pack(side=tk.LEFT)
        self.folder_var = tk.StringVar(value=self.config.get("watchFolder", ""))
        ttk.Entry(folder_frame, textvariable=self.folder_var, width=40).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(api_frame, text="API SisFin:", width=12).pack(side=tk.LEFT)
        self.api_var = tk.StringVar(value=self.config.get("apiUrl", DEFAULT_CONFIG["apiUrl"]))
        ttk.Entry(api_frame, textvariable=self.api_var, width=40).pack(side=tk.LEFT, padx=5)

        # Schedule
        sched_frame = ttk.Frame(main)
        sched_frame.pack(fill=tk.X, pady=2)
        ttk.Label(sched_frame, text="Horário:", width=12).pack(side=tk.LEFT)
        self.schedule_var = tk.StringVar(value=self.config.get("scheduleTime", "18:00"))
        ttk.Entry(sched_frame, textvariable=self.schedule_var, width=10).pack(side=tk.LEFT, padx=5)
        ttk.Label(sched_frame, text="(verificação diária)").pack(side=tk.LEFT)

        # Buttons
        btn_frame = ttk.Frame(main)
        btn_frame.pack(fill=tk.X, pady=10)
        self.btn_start = ttk.Button(btn_frame, text="▶ Monitorar", command=self.start_monitoring)
        self.btn_start.pack(side=tk.LEFT, padx=5)
        self.btn_stop = ttk.Button(btn_frame, text="⏸ Parar", command=self.stop_monitoring, state=tk.DISABLED)
        self.btn_stop.pack(side=tk.LEFT, padx=5)
        self.btn_check_now = ttk.Button(btn_frame, text="Verificar Agora", command=self.check_now)
        self.btn_check_now.pack(side=tk.LEFT, padx=5)

        # Status
        self.status_var = tk.StringVar(value="● Parado")
        self.status_label = ttk.Label(main, textvariable=self.status_var, font=("Segoe UI", 10))
        self.status_label.pack(anchor=tk.W)

        self.last_check_var = tk.StringVar(value="Última verificação: —")
        ttk.Label(main, textvariable=self.last_check_var, font=("Segoe UI", 9)).pack(anchor=tk.W)

        # Log
        log_label = ttk.Label(main, text="Log:", font=("Segoe UI", 9, "bold"))
        log_label.pack(anchor=tk.W, pady=(5, 0))

        log_frame = ttk.Frame(main)
        log_frame.pack(fill=tk.BOTH, expand=True)

        self.log_text = tk.Text(log_frame, height=12, font=("Consolas", 9), state=tk.DISABLED, bg="#1e1e1e", fg="#d4d4d4", insertbackground="white")
    ...
    def check_folder(self):
        ...
        if not folder or not os.path.isdir(folder):
            self.log("❌ Pasta não configurada ou não encontrada")
            return
        ...
        for file_name in xml_files:
            ...
            try:
                ...
                self.log(f"❌ {file_name} → Erro: {e}")
                self.stats["errors"] += 1
                self.update_stats()

        self.last_check = datetime.now()
        self.last_check_var.set(f"Última verificação: {self.last_check.strftime('%H:%M:%S')} — {self.stats['imported']} importados")
    ...
    def start_monitoring(self):
        ...
        if not os.path.isdir(folder):
            self.log("❌ Pasta não encontrada: " + folder)
            return
